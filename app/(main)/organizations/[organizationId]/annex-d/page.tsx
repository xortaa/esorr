...
export default function AnnexDManager({ params }: { params: { organizationId: string } }) {
  ...
  const fetchAnnexes = async () => {
    try {
      const response = await axios.get(`/api/annexes/${params.organizationId}/annex-d`);
  ...
  const submitAnnexForReview = async (id: string) => {
    try {
      const response = await axios.patch(`/api/annexes/${params.organizationId}/annex-d/${id}`, {
        isSubmitted: true,
      });
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <h2 className="card-title">
              Organizations Logo and Letterhead  Annex for AY {annex.academicYear}
            </h2>
          </div>
