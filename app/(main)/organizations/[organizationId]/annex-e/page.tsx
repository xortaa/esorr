...
export default function AnnexeManager({ params }: { params: { organizationId: string } }) {
  ...
  const fetchAnnexes = async () => {
    try {
      const response = await axios.get(`/api/annexes/${params.organizationId}/annex-e`);
  ...
  const submitAnnexForReview = async (id: string) => {
    try {
      const response = await axios.patch(`/api/annexes/${params.organizationId}/annex-e/${id}`, {
        isSubmitted: true,
      });
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <h2 className="card-title">
              Organization Operational Assessment Form Annex for AY {annex.academicYear}
            </h2>
          </div>
