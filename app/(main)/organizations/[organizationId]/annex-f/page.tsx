...
export default function AnnexFManager({ params }: { params: { organizationId: string } }) {
  ...
  const fetchAnnexes = async () => {
    try {
      const response = await axios.get(`/api/annexes/${params.organizationId}/annex-f`);
  ...
  const submitAnnexForReview = async (id: string) => {
    try {
      const response = await axios.patch(`/api/annexes/${params.organizationId}/annex-f/${id}`, {
        isSubmitted: true,
      });
  ...
  return (
    <PageWrapper>
      <h1 className="text-2xl font-bold mb-6">ANNEX F Activities_ Monitoring Form</h1>
      ...
      {annexList.length === 0 && (
        <div className="text-center text-gray-500 mt-8">
          <p>No Activities_ Monitoring Form Annex created yet.</p>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            ...
            <h2 className="card-title">
              Activities_ Monitoring Form Annex for AY {annex.academicYear}
            </h2>
          </div>
