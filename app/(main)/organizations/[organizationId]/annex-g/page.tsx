...
export default function AnnexGManager({ params }: { params: { organizationId: string } }) {
  ...
  const fetchAnnexes = async () => {
    try {
      const response = await axios.get(`/api/annexes/${params.organizationId}/annex-g`);
  ...
  const submitAnnexForReview = async (id: string) => {
    try {
      const response = await axios.patch(`/api/annexes/${params.organizationId}/annex-g/${id}`, {
        isSubmitted: true,
      });
    ...
    <PageWrapper>
      ...
      {annexList.length === 0 && (
        <div className="text-center text-gray-500 mt-8">
          <p>No Organization Adviser Nomination Form Annex created yet.</p>
