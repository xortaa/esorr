...
export default function EnhancedAnnexIManager() {
  ...
  const submitAnnexForReview = async (id: string) => {
    try {
      const response = await axios.patch(`/api/annexes/${organizationId}/annex-i/${id}`, { isSubmitted: true });
  ...
  const openSignatureModal = async (annex: AnnexI) => {
    try {
      setIsLoading(true);
  ...
  const handleSubmitSignature = async () => {
    ...
    try {
      ...
      const updateResponse = await axios.patch(`/api/annexes/${organizationId}/annex-i/${selectedAnnex._id}`, {
      ...
      if (updateResponse.data) {
        ...
        throw new Error("Failed to update Annex I");
      }
    } catch (error) {
          ...
          <div className="relative w-auto max-w-7xl mx-auto my-6">
            <div className="relative flex flex-col w-full bg-white border-0 rounded-lg shadow-lg outline-none focus:outline-none">
              <div className="flex items-start justify-between p-5 border-b border-solid rounded-t">
                <h3 className="text-2xl font-semibold">Add Signature to Annex I</h3>
                <button
