  ...
  <Document>
    <Page style={styles.page} size="LEGAL" orientation="portrait">
      <View fixed style={styles.header}>
        ...
        <Text style={{ fontSize: 8, fontFamily: "Arial Narrow Bold", textAlign: "right" }}>ANNEX J</Text>
      ...
      <View style={styles.section}>
        <Text style={styles.heading}>COMMITMENT TO ACTIVE PARTICIPATION</Text>
      </View>
      ...
      <View style={styles.section}>
        <Text style={{ fontSize: 11, textAlign: "center", color: "red", textDecoration: "underline" }}>
          ...
          <Text style={{ fontFamily: "Arial Narrow Bold" }}>
            ACTIVE PARTICIPATION IN ALL UNIVERSITY OR OSA-INITIATED ACTIVITIES
          </Text>
        </Text>
      </View>
      ...
      <View style={styles.section}>
        <Text style={styles.text}>
          We, the officers of {annex.organization.name} are expected to attend and actively participate, or assure that
          a member of the organization shall be made available to participate in all OSA-initiated activities
          (specifically the Valiant Legion Tool) and advocacies of the University. We understand that failure to attend
          to any activity for any reason shall result in sanction or denial of our recognition the following Academic
          Year ({annex.academicYear}).
        </Text>
      </View>
...
export default function EnhancedAnnexJManager() {
  const { data: session } = useSession();
  const [annexList, setAnnexList] = useState<AnnexJ[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [selectedAnnex, setSelectedAnnex] = useState<AnnexJ | null>(null);
  ...
  const fetchAnnexes = async () => {
    setIsLoading(true);
    try {
      const response = await axios.get(`/api/annexes/${organizationId}/annex-j`);
  ...
  const submitAnnexForReview = async (id: string) => {
    try {
      const response = await axios.patch(`/api/annexes/${organizationId}/annex-j/${id}`, { isSubmitted: true });
  ...
  const openSignatureModal = async (annex: AnnexJ) => {
    try {
      setIsLoading(true);
  ...
  const generatePDFBlob = async (annex: AnnexJ): Promise<Blob> => {
    try {
  ...
  const generatePDF = async (annex: AnnexJ) => {
    try {
      setIsLoading(true);
      const updatedAnnex = await fetchUpdatedAnnex(annex._id);
      const blob = await generatePDFBlob(updatedAnnex);
  ...
  const fetchUpdatedAnnex = async (annexId: string): Promise<AnnexJ> => {
    if (!organizationId) {
      throw new Error("Organization ID is not available");
    }
    const response = await axios.get(`/api/annexes/${organizationId}/annex-j/${annexId}`);
  ...
  const handleSubmitSignature = async () => {
    ...
    try {
      ...
      const updateResponse = await axios.patch(`/api/annexes/${organizationId}/annex-j/${selectedAnnex._id}`, {
      ...
      if (updateResponse.data) {
        ...
        throw new Error("Failed to update Annex J");
      }
    } catch (error) {
  ...
  const handleSubmitAnnex = async (annexId: string) => {
    try {
      const response = await axios.post(`/api/annexes/${organizationId}/annex-j/${annexId}/submit`);
  ...
  const handleUpdateRemarks = async (annexId: string, type: "socc" | "osa", remarks: string) => {
    try {
      const response = await axios.post(`/api/annexes/${organizationId}/annex-j/${annexId}/${type}-remarks`, {
        remarks,
      });
      const updatedAnnex = response.data;
      setAnnexList(annexList.map((annex) => (annex._id === updatedAnnex._id ? updatedAnnex : annex)));
  ...
  const handleApprove = async (annexId: string) => {
    try {
      const response = await axios.post(`/api/annexes/${organizationId}/annex-j/${annexId}/approve`);
      const updatedAnnex = response.data;
      setAnnexList(annexList.map((annex) => (annex._id === updatedAnnex._id ? updatedAnnex : annex)));
  ...
  const handleDisapprove = async (annexId: string) => {
    try {
      const response = await axios.post(`/api/annexes/${organizationId}/annex-j/${annexId}/disapprove`);
      const updatedAnnex = response.data;
      setAnnexList(annexList.map((annex) => (annex._id === updatedAnnex._id ? updatedAnnex : annex)));
        ...
        <div className="space-y-4">
          ...
          {annexList.length === 0 && (
            <div className="text-center text-gray-500 mt-8">
              <p>No COMMITMENT TO ACTIVE PARTICIPATION Annex created yet.</p>
          ...
          <div className="relative w-auto max-w-7xl mx-auto my-6">
            <div className="relative flex flex-col w-full bg-white border-0 rounded-lg shadow-lg outline-none focus:outline-none">
              <div className="flex items-start justify-between p-5 border-b border-solid rounded-t">
                <h3 className="text-2xl font-semibold">Add Signature to Annex J</h3>
                <button
...
interface AnnexCardProps {
  annex: AnnexJ;
  openSignatureModal: (annex: AnnexJ) => void;
  generatePDF: (annex: AnnexJ) => void;
