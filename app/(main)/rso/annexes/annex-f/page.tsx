...
export default function AnnexFManager() {
  ...
  const editAnnex = (id: string) => {
    ...
    router.push(`/rso/annexes/annex-f/${id}`);
  };

  return (
    <PageWrapper>
      <h1 className="text-2xl font-bold mb-6">Annex F Activities Monitoring Form Annex Manager</h1>
      <button onClick={createNewAnnex} className="btn btn-primary mb-6">
        <Plus className="mr-2 h-4 w-4" />
        Create New Activities Monitoring Form Annex
      </button>
          ...
          <div key={annex.id} className="card bg-base-100 shadow-xl">
            <div className="card-body">
              <div className="flex items-center justify-between">
                <h2 className="card-title">Activities Monitoring Form Annex for Year {annex.year}</h2>
