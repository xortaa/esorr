...
export async function POST(request: NextRequest, { params }: { params: { organizationId: string; annexId: string } }) {
  try {
    ...
    const updatedAnnex = await AnnexC1.findOneAndUpdate(
    ...
    await Notification.create({
      text: `${updatedAnnex.organization.name} has submitted Annex C1 for review`,
      date: new Date(),
      link: `/organizations/${organizationId}/annexC1`,
