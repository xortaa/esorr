...
export async function POST(request: NextRequest, { params }: { params: { organizationId: string; annexId: string } }) {
  try {
    ...
    const updatedAnnex = await AnnexD.findOneAndUpdate(
    ...
    await Notification.create({
      text: `${updatedAnnex.organization.name} has submitted Annex D for review`,
      date: new Date(),
      link: `/organizations/${organizationId}/annexD`,
