...
export async function POST(request: NextRequest, { params }: { params: { organizationId: string; annexId: string } }) {
  try {
    ...
    const updatedAnnex = await AnnexG.findOneAndUpdate(
    ...
    await Notification.create({
      text: `${updatedAnnex.organization.name} has submitted Annex G for review`,
      date: new Date(),
      link: `/organizations/${organizationId}/annexG`,
